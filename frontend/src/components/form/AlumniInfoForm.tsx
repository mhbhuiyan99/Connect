...
const AlumniInfoForm = ({ mode = "insert", initialData }: AlumniInfoFormProps) => {
  ...
  const [companySections, setCompanySections] = useState<Company[]>(
  ...
  const presetIndustries = ["Enosis Solutions", "Cefalo", "Dynamic Solution Innovators", "TigerIT", "Banglalink", "Pathao"];
  ...
  const handleRemoveCompany = (index: number) => {
    const updated = companySections.filter((_: Company, i: number) => i !== index);
    const updatedVisibility = showExtraFieldsFor.filter((_: boolean, i: number) => i !== index);
      ...
      <Card className="w-[80%] max-w-4xl p-6">
        ...
        <CardContent>
          <form
            ...
            {companySections.map((company: Company, idx: number) => (
              <CompanySection
                key={idx}
                index={idx}
                company={company}
